...
besoin de : iprox (faire une nouvelle branche), linear operators ma branche, un code qui implémente un pb type BPDN et NNMF
...
function TR_diagHess(
  ...
  selected::UnitRange{T} = 1:length(f.meta.x0)) where {T <: Integer}
